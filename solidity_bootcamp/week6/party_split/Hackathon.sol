...
contract Hackathon {
    ...
    function findWinner() external view returns (Project memory) {
        uint maxAverageRating = 0;
        uint maxAverageRatingProjectIdx = 0;
        for (uint i = 0; i < projects.length; i++) {
            uint[] memory ratings = projects[i].ratings;
            uint sumOfRating = 0;
            for (uint j = 0; j < ratings.length; j++) {
                sumOfRating += ratings[j];
            }
            uint averageRating = sumOfRating / ratings.length;
            if (averageRating > maxAverageRating) {
                maxAverageRating = averageRating;
                maxAverageRatingProjectIdx = i;
            }
        }
        return projects[maxAverageRatingProjectIdx];
    }
}
